...
sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)  # in the brackets optional info, could be skipped
sock.bind((HOST, PORT))
sock.listen(1)
...
def waiting_connection():
    ...
    conn, addr = sock.accept()  # a blocking method, waiting for connection
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and connection_established:
            if pygame.mouse.get_pressed()[0]:
                if turn and not grid.game_over:
                    ...
                    cellX, cellY = position[0] // 200, position[1] // 200
